...
type CommRecord struct {
	ID             uint      `gorm:"primaryKey;autoIncrement;index:idx_comm_records_group_status_start_id,priority:4" json:"id"`
	...
	GroupID        *uint     `gorm:"index;index:idx_group_start,priority:1;index:idx_comm_records_group_status_start_id,priority:1;column:group_id" json:"group_id"`                                              // 性能优化：复合索引用于按群组查询
	UserID         *uint     `gorm:"index;index:idx_user_start,priority:1;column:user_id" json:"user_id"`                                                                                                         // 性能优化：复合索引用于按用户查询
	StartTime      time.Time `gorm:"index;index:idx_group_start,priority:2;index:idx_user_start,priority:2;index:idx_comm_records_group_status_start_id,priority:3;not null;column:start_time" json:"start_time"` // 性能优化：复合索引
	...
	Status         int       `gorm:"default:0;index;index:idx_comm_records_group_status_start_id,priority:2;column:status" json:"status"`                                                                         // 状态：0=录制中,1=待上传,2=已完成,3=上传失败
	MessageType    uint8     `gorm:"type:tinyint unsigned;not null;default:0;column:message_type" json:"message_type"`                                                                                            // 0=语音, 1=文本
