...
管理员后台用户列表提供“以该用户身份登录”。确认后，前端会直接用目标用户的
access token 和用户资料覆盖当前管理员登录态，refresh/WebSocket Cookie 也由后端替换。
该操作不是模拟登录，不保存原管理员 token，也不能一键返回；切回管理员账号需要重新登录。
普通用户不能调用该接口，目标账号必须启用，且主管理员 ID 1 不能作为切换目标。
