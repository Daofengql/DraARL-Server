...
WebSocket 同时承载两类消息：服务端文本控制事件，以及双向 DraARLv1 二进制帧。
客户端必须先处理文本控制事件（例如 `auth_success`、`routing_updated`），再处理媒体帧；
不能把控制 JSON 当作协议错误或交给二进制解码器。
...
如果启用音频记录，音频数据会写入当前 `Storage` 驱动（local、MinIO 或其他 S3 兼容
profile），索引和统计信息写入数据库。S3 读取使用预签名 URL，不应开放 bucket 匿名访问。
