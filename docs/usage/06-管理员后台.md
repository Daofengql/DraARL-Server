...
通用 artifact 先浏览器直传 staging，后端流式计算 SHA-256 后提升为不可变对象。同一文件可绑定多个明确 target，但没有 `universal` 通配。published 版本可撤回但不能编辑或删除，只有 draft 可删除。资源页会显示当前管理员待完成的 staging 上传，可重新签发 complete 凭证继续提交；发布确认会展示 final key、大小、SHA-256 和目标范围，撤回确认会列出将回退的上一发布版本或提示无可回退版本。
...
| POST | `/api/client-resources/:resource_id/releases/:release_id/artifacts/complete` | Admin | 完成 artifact 直传。 |
| POST | `.../:release_id/publish` | Admin | 发布草稿。 |
| POST | `.../:release_id/withdraw` | Admin | 撤回发布。 |
