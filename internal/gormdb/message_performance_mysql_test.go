...
func TestMillionRowMessageCursorPlanMySQL(t *testing.T) {
	...
	if err := tx.Exec(
		"INSERT INTO comm_record_delivery_groups (record_id, group_id) SELECT id, group_id FROM comm_records WHERE user_id = ?",
		user.ID,
	).Error; err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(plan, "idx_delivery_group_record") {
	...
	if !strings.Contains(typePlan, "idx_delivery_group_record") {
...
func explainMessageCursorPlan(t *testing.T, db *gorm.DB, groupID int, before time.Time) string {
	...
	rows, err := db.Raw(`
		...
		FROM comm_record_delivery_groups dg FORCE INDEX (idx_delivery_group_record)
...
func explainMessageTypeCursorPlan(t *testing.T, db *gorm.DB, groupID int, before time.Time, messageType uint8) string {
	...
	rows, err := db.Raw(`
		...
		FROM comm_record_delivery_groups dg FORCE INDEX (idx_delivery_group_record)
		INNER JOIN comm_records cr ON cr.id = dg.record_id
		WHERE cr.status = 2 AND dg.group_id = ? AND cr.message_type = ?
			AND cr.start_time < ?
		ORDER BY cr.start_time DESC, cr.id DESC
		LIMIT 51
