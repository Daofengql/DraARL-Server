...
func ParsePreAuthData(r *http.Request) *WSPreAuthData {
	...
	// 优先读取专用 ws_token Cookie（浏览器流程，由后端 Set-Cookie 注入 HttpOnly）
	if cookie, err := r.Cookie(wsTokenCookieName); err == nil && cookie.Value != "" {
		data.Token = cookie.Value
	}

	// Cookie 为空时回退到 URL query parameter（小程序流程：wss://host/ws?token=xxx）
	if data.Token == "" {
