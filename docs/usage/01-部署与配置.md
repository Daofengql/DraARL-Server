...
```bash
# 预览：只统计不写入
./draarl -c udphub.yaml -migrate-storage minio:local -migrate-dry-run

# 正式迁移（保留源端，便于验证）
./draarl -c udphub.yaml -migrate-storage minio:local

# 验证无误后，迁移并删除源端对象
./draarl -c udphub.yaml -migrate-storage minio:local -migrate-delete-source
```
