...
func (s *Server) setupRoutes() {
	...
	api := s.engine.Group("/api")
	{
		...
		auth := api.Group("/auth")
		{
			...
			auth.POST("/wechat-mini/login", handler.WeChatMiniLogin)
		}
		...
		protected.Use(middleware.LoadUserInfo()) // 加载用户ID到context
		{
			...
			protected.GET("/sso/wechat-mini/status", handler.WeChatMiniStatus)
			protected.POST("/sso/wechat-mini/bind", handler.WeChatMiniBind)
			protected.DELETE("/sso/wechat-mini/unbind", handler.WeChatMiniUnbind)
