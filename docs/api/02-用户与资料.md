...
- 该接口不支持直接修改 `callsign`。
- 如需变更呼号，请通过 `POST /api/upload/operator-certificate` 提交操作证/呼号申请，并等待管理员审批通过后生效。
...
管理员用户管理不提供 `POST /api/users` 创建入口。新账号统一通过
`POST /api/auth/register` 注册，再由管理员审批或修改状态；管理员只能使用下列
已登记的列表、详情、更新、删除和状态接口。
...
- 该接口不支持管理员直接修改 `callsign`。
- 管理员如需处理呼号变更，应审批用户提交的操作证申请。
