...
func handlePacket(device *WSDevice, packet *WSPacket, rawData []byte) {
	...
	case protocol.DraARLTypePCM16K:
		handleVoice(device, packet, rawData) // PCM 在 handleVoice 内部转码
...
func handleVoice(device *WSDevice, packet *WSPacket, rawData []byte) {
	// PCM 转码：如果收到 PCM 16K 数据包，先转码为 Opus
	if packet.Type == protocol.DraARLTypePCM16K {
		encoder, err := codec.GetOpusEncoder()
		if err != nil {
			log.Printf("[WS] Opus 编码器不可用: %v", err)
			return
		}
		opusData, err := encoder.EncodePCMToOpus(packet.DATA)
		if err != nil {
			log.Printf("[WS] PCM 转码失败: %v", err)
			return
		}
		packet.DATA = opusData
		packet.Type = protocol.DraARLTypeOpus16K
	}
