...
| GET | `/api/admin/users/:id/device-password` | Admin | 按需读取设备准入密码，响应禁止缓存。 |
...
| DELETE | `/api/admin/radio/sessions/:session_id` | Admin | 精确断开一个 Session。 |
| GET | `/api/edge-nodes` | Admin | 边缘节点配置、在线状态、流量和投影指标。 |
| POST | `/api/edge-nodes` | Admin | 创建边缘节点并签发一次性注册凭据。 |
| PUT | `/api/edge-nodes/:id` | Admin | 更新节点和公网接入点字段。 |
| DELETE | `/api/edge-nodes/:id` | Admin | 删除节点并使凭据失效。 |
...
通用 artifact 先浏览器直传 staging，后端流式计算 SHA-256 后提升为不可变对象。同一文件可绑定多个明确 target，但没有 `universal` 通配。已发布版本可进入编辑状态追加不冲突的 artifact，适合先发布 Android 再补充 Windows；已有 artifact 不可覆盖。删除版本会立即停止其 manifest 分发，并清理关联对象。资源页会显示当前管理员待完成的 staging 上传，可重新签发 complete 凭证继续提交。

创建版本时可以填写最低客户端版本、最低服务端版本、所需幽灵协议版本和服务端能力。发布确认会同时展示这些契约以及 final key、大小、SHA-256 和目标范围。当前服务端版本、协议或能力不满足时发布会被拒绝；`dev` 构建无法发布声明最低服务端 SemVer 的版本。
...
| GET | `/api/udp/metrics` | Admin | UDP、fan-out、幽灵 Session、WebSocket 和消息 API 指标。 |
