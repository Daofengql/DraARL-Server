...
本文对应 DraARL Server `v2.0.0-alpha3`。报文线格式仍以固定字符串 `DraA` 标识；
...
1. **群组消息**：发送到设备当前所属群组的所有在线设备
2. **系统消息**：由服务器生成，广播到所有在线设备
3. **紧急消息**：优先显示，可触发设备端告警
...
- **固定头部保持不变**：MAC 仅允许追加在心跳包 `DATA` 区域，90 字节 header 的字段、偏移、长度均不变。
- `DATA[24:]` 为可选扩展区；当前约定可放置设备 MAC 文本，格式为 ASCII 大写冒号分隔，例如 `AA:BB:CC:DD:EE:FF`。
- 旧设备如果不携带 MAC，服务端仍按旧规则兼容处理。
...
![普通设备认证流程](assets/diagrams/device-auth-flow.svg)
...
![设备动态码绑定流程](assets/diagrams/device-binding-flow-detailed.svg)
...
![语音转发流程](assets/diagrams/voice-forwarding.svg)
