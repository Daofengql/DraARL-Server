...
| 现代幽灵设备(App/PC) | 按客户端实例和临时 Session 隔离 | 同账号同平台可多端；同一安装实例重连只替换自身旧 Session |
| 现代 WebSocket(Web) | 每个浏览器安装实例独立在线 | 多标签页/多浏览器不会覆盖其他移动端或浏览器实例 |
| legacy 幽灵设备 | 同账号同平台只允许一个 | 未提交 `client_instance_id` 的旧客户端保持旧冲突规则和单频道能力 |

现代幽灵 Session 采用“多收单发”：每个 Session 只有一个发送频道，同时可订阅最多 16 个接收频道。标准 UDP 实体设备不进入该 Session 模型，继续保持 `owner_id + ssid` 单端约束。
