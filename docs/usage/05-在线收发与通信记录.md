...
- 展示当前发言人。
- 展示连接状态。
- 同账号多个现代 Web、Android 或其他幽灵客户端同时在线。

使用注意：

- 页面需要麦克风和音频播放权限。
- 浏览器会持久化随机 `client_instance_id`；现代会话不会因同账号其他设备上线而断开。
- 发送频道会自动包含在接收集合中；切换发送频道不会取消其他已选收听频道。
- 单个现代会话最多接收 16 个频道。未携带实例 ID 和能力声明的 legacy 客户端仍为同平台单端、单频道。
...
| PUT | `/api/radio/group` | JWT+Approved | legacy 切组兼容接口；多会话时应明确提交 Session。 |
...
认证依赖 Cookie，无需也不能在 URL 中附加 token。现代客户端还会提交 `client_instance_id`、`protocol_version=1` 以及 `multi_receive_v1,source_group_v1` 能力；认证成功后服务端以文本控制消息返回 `session_id` 和完整路由状态。
...
通信记录用于本人发信记录和审计，不等同于频道聊天历史。频道页面通过 `GET /api/groups/:id/messages` 按群组 ACL 查询所有可见发送者的已完成消息，使用 `cursor` 加载更早内容；单条消息和授权音频地址通过 `GET /api/groups/:id/messages/:message_id` 获取。
...
| GET | `/api/groups/:id/messages` | JWT+Approved | 按频道查询语音/文本历史。 |
| GET | `/api/groups/:id/messages/:message_id` | JWT+Approved | 查询频道消息详情和授权音频地址。 |
