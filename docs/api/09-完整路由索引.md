...
| GET | /healthz | Public | 进程存活检查，返回 `status=ok` |
...
| GET | /api/users | Admin | 管理员分页查询用户 |
| PUT | /api/users/:id | Admin | 管理员更新用户资料/角色/状态 |
| DELETE | /api/users/:id | Admin | 删除用户及其级联数据 |
| PUT | /api/users/:id/status | Admin | 更新用户启用状态 |
| GET | /api/users/:id | Admin | 管理员查看用户详情 |
| GET | /api/admin/users/:id/device-password | Admin | 按需读取用户设备准入密码（`no-store`） |
...
| DELETE | /api/groups/:id/members/:userId | Admin/Owner | 移除私有群组成员并立即撤销其收发与历史权限 |
...
| GET | /api/groups/:id/broadcast-context | Admin/Owner | 自动播报互联策略与当前允许状态 |
| GET | /api/groups/:id/broadcast-audios | Admin/Owner | 自动播报音频库 |
| POST | /api/groups/:id/broadcast-audios | Admin/Owner | 上传自动播报音频（multipart） |
| GET | /api/groups/:id/broadcast-audios/:audioId | Admin/Owner | 音频详情和短时试听地址 |
| DELETE | /api/groups/:id/broadcast-audios/:audioId | Admin/Owner | 删除未被计划引用的音频 |
| GET | /api/groups/:id/broadcast-schedules | Admin/Owner | 自动播报计划列表 |
| POST | /api/groups/:id/broadcast-schedules | Admin/Owner | 创建播报计划 |
| PATCH | /api/groups/:id/broadcast-schedules/:scheduleId | Admin/Owner | 更新播报计划 |
| DELETE | /api/groups/:id/broadcast-schedules/:scheduleId | Admin/Owner | 删除播报计划 |
| POST | /api/groups/:id/broadcast-schedules/:scheduleId/run | Admin/Owner | 手动触发一次播报 |
| GET | /api/groups/:id/broadcast-runs | Admin/Owner | 播报执行历史 |
| POST | /api/groups/:id/broadcast-runs/:runId/cancel | Admin/Owner | 停止已认领或播放中的播报 |
...
| PUT | /api/group-links/:id/broadcast-policy | Admin | 更新互联期间的自动播报策略 |
...
| GET | /api/admin/radio/sessions | Admin | 全部在线幽灵 Session 摘要 |
| DELETE | /api/admin/radio/sessions/:session_id | Admin | 精确断开任意一个幽灵 Session |
...
| GET | /api/admin/groups | Admin | 管理员全量群组列表 |
...
| GET | /api/broadcast/metrics | Admin | 自动播报持久化、调度器和媒体指标 |
| GET | /api/broadcast/health | Admin | 自动播报调度器与媒体健康状态 |
| PUT | /api/broadcast/runtime | Admin | 开关站点自动播报运行态 |
| POST | /api/broadcast/emergency-stop | Admin | 紧急停止当前全部自动播报 |
...
| PUT | /api/config/access-discovery | Admin | 更新接入点发现配置 |
| GET | /api/config/registration | Admin | 注册配置读取 |
| PUT | /api/config/registration | Admin | 更新注册配置 |
...
| GET | /api/storage/get | Token | local 驱动受控下载；只接受短期签名参数 |
