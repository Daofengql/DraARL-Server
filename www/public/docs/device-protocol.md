...
本文对应 DraARL Server `v2.0.0-alpha6`，面向设备/客户端开发者提供精简接入说明；
...
- 固定头部始终保持 `90` 字节不变；MAC 不能写入 header。
- 如需上报 MAC，只能可选追加在心跳 `DATA[24:]`，格式为 ASCII 大写冒号分隔，例如 `AA:BB:CC:DD:EE:FF`。
- 旧设备不携带 MAC 也可继续工作。
- 服务端收到合法 MAC 后，会将其作为“同一物理设备快速重连”的辅助判据。
...
| **幽灵/保留段** | 100-105 | `100` 预留；`101-105` 为 JWT Token | `100` 为预留位；`101-104` 为 App/PC 客户端；`105` 为 Web |
| **普通设备（无输入）** | 1-99, 106-254 | 动态码绑定 | 新设备首次绑定，适用于无键盘/触摸屏的嵌入式设备 |
