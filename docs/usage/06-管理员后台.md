...
`/admin/client-releases` 用于发布 Windows、macOS、Android 和 iOS 客户端安装包：

- 新建 `draft`，填写应用 ID、严格 SemVer 2.0.0 版本、频道、更新日志、最低支持版本与强制更新标记；最低支持版本不能高于发布版本。
- 在草稿中逐个添加目标 artifact。Android 必须分别上传 `armv7` 和 `arm64`，或明确上传 `universal`；不会跨 ABI 自动回退。iOS App Store/TestFlight 外部发布固定使用 `universal`。
- 安装包先浏览器直传 staging，后端校验大小、类型并计算 SHA-256。管理页面显示每个版本的发布矩阵。
- 草稿完成后发布。已发布版本可撤回但不能修改或删除；只有草稿可删除。

客户端检查接口、支持的包类型、iOS App Store/TestFlight 外部地址和 ETag 细节见 [客户端发布与更新](../api/12-客户端发布与更新.md)。
